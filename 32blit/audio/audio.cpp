...
#define BLIT_AUDIO_FUNC(name) __not_in_flash_func(name)
#else
#define BLIT_AUDIO_FUNC(name) name
#endif
...
namespace blit {
  ...
  uint32_t BLIT_AUDIO_FUNC(prng_xorshift_next)() {
  ...
  int32_t BLIT_AUDIO_FUNC(prng_normal)() {
  ...
  uint16_t BLIT_AUDIO_FUNC(get_audio_frame)() {
    int32_t sample = 0;  // used to combine channel output

    // Cache the HAL channel pointer in SRAM so the mix loop does not hit
    // api.channels in flash on every sample (that stalls core 0's XIP).
#if defined(PICO_ON_DEVICE) && PICO_ON_DEVICE
    static AudioChannel *chans = nullptr;
    if(!chans)
      chans = channels;
#else
    AudioChannel *chans = channels;
#endif

    for(int c = 0; c < CHANNEL_COUNT; c++) {

      auto &channel = chans[c];
      ...
      if(channel.waveforms) {
        ...
        // Skip expf() on Pico: it lives in XIP and would stall the RAM audio core.
#if !(defined(PICO_ON_DEVICE) && PICO_ON_DEVICE)
